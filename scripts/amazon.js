...
document.querySelectorAll(".js-add-to-cart").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const addedProduct = document.querySelector(`.js-added-${productId}`);
    ...
    /* 
      ...
      we are saving in a object the product id and timeout id to check if exists
      and then cleaning it, so the message can stay on screen for the correct time
    */
    ...
    const timeoutId = setTimeout(() => {
      addedProduct.classList.remove("js-show-added-message");
    }, 2000);
